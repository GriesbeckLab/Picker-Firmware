...
float plate_colony() {
  ...
  enable_endstops(true);
  float start_x = current_position[X_AXIS];
  float start_y = current_position[Y_AXIS];
  float start_z = current_position[Z_AXIS];
  long start_steps = st_get_position(Z_AXIS);
  ...
  destination[X_AXIS] = start_x;
  destination[Y_AXIS] = start_y + 0.5;
  prepare_move_raw();
  
  destination[X_AXIS] = start_x + 0.5;
  destination[Y_AXIS] = start_y + 0.5;
  prepare_move_raw();
  ...
  destination[X_AXIS] = start_x - 0.5;
  destination[Y_AXIS] = start_y + 1;
  prepare_move_raw();
  
  destination[X_AXIS] = start_x + 1;
  destination[Y_AXIS] = start_y + 1;
  prepare_move_raw();
  ...
  destination[X_AXIS] = start_x - 1;
  destination[Y_AXIS] = start_y + 1.5;
  prepare_move_raw();
  
  
  destination[X_AXIS] = start_x + 1.5;
  destination[Y_AXIS] = start_y + 1.5;
  prepare_move_raw();
  ...
  destination[X_AXIS] = start_x - 1.5;
  destination[Y_AXIS] = start_y + 2;
  prepare_move_raw();
  
  destination[X_AXIS] = start_x + 2;
  destination[Y_AXIS] = start_y + 2;
  prepare_move_raw();
...
void process_commands()
{
  ...
  if(code_seen('G'))
  {
    ...
    case 30: // G30 Crash Z probe into colony.
      pick_colony();
      break;
      
    case 31: // G31 Plate colonies with Z-probe
      plate_colony();
      break;  
