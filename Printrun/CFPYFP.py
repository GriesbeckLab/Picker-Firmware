...
data = numpy.memmap(plate_name, dtype='uint16', mode='w+', shape=(30,1040,4176))
...
for i in range(10):
    ...
    if t <= timePeriod:
        delay = timePeriod-t
        time.sleep(delay)
    print "Recorded %d of 10" %(i+1)


# Prompts the user to add cGMP, then records 25 data points for each colony, at 15 second intervals.
tkMessageBox.showinfo("Command", "Add cGMP")
print "continuing after cGMP addition"
