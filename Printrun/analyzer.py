...
def remove_background(data):
    ...
    no_background = data / scipy.ndimage.median_filter(data, 10) * numpy.mean(data) #scipy.ndimage.median_filter(data, 5) / scipy.ndimage.median_filter(data, 30) * numpy.mean(data)
...
def remove_background_beads(data):
    """Remove background and stretchs data to 8-bit range"""
    # convert to floating point (integers lose 99% of data!)
    data = data.astype(float)
    data = numpy.fabs(4096 - data)
...
def process_image(data):
    ...
    for i in range(1, count):
        if properties[i - 1]['Area'] < 30:
            remove_object[i] = True
        if (4 * math.pi * properties[i - 1]['Area'] /
                (properties[i - 1]['Perimeter'] * properties[i - 1]['Perimeter']) <= 0.8):
            remove_object[i] = True
